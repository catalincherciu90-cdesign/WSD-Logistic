...
messaging.onBackgroundMessage(function(payload) {
    const title = payload.notification?.title || 'DepanAuto';
    const body  = payload.notification?.body  || 'Actualizare cerere.';
    ...
    event.waitUntil(
        clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function(clientList) {
            for (const client of clientList) {
                if (client.url.includes('/depanauto/client/') && 'focus' in client) {
                    return client.focus();
                }
            }
            if (clients.openWindow) return clients.openWindow('/depanauto/client/');
        })
    );
});
